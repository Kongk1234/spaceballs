...
export default class NavigationComponent extends Component {
  ...
  @action async edit(item) {
    ...
    document
      .getElementById(item.id + '-description')
      .removeAttribute('readonly');
    document.getElementById(item.id + '-price').removeAttribute('readonly');
    document
      .getElementById(item.id + '-category_id')
      .removeAttribute('readonly');
    ...
    button.addEventListener('click', async () => {
      item.amount = parseInt(
        document.getElementById(item.id + '-amount').value
      );
      item.name = document.getElementById(item.id + '-name').value;
      item.description = document.getElementById(
        item.id + '-description'
      ).value;
      item.price = parseFloat(
        document.getElementById(item.id + '-price').value
      );
      (item.category_id = parseInt(
        document.getElementById(item.id + '-category_id').value
      )),
